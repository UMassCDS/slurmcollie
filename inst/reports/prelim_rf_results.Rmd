...
predict <- 'predict_oth_2025-Apr-28_13-54'
...
See the Appendix for a list of subclasses ("class" in the stats table above).
...
- For each class (the 19 subclasses), we get the following statistics. These all scale from 0 to 1, with 1 
  generally indicating higher performance (except for prevalence, detection rate, and detection prevalence).
  
  - *Precision*, the proportion of cases predicted to be in the class that are correct
  - *Recall*, the proportion of cases in the class that were predicted to be in the class
...
none of them look bad in this example. We're fitting the transitional marsh subclasses (3, 4, 5) fairly well,
though subclass 4 could use some improvement (though predicting classes along a gradient is inherently tricky).
...
by hydrology. 
- This particular fit photogrammetry-based DEMs in the two top positions, with one in April and
one in August. DEMs representing other seasons come in later. Other runs I've tried have early-late 
pairs high up too. I interpret this as the model using this combination to get at canopy height. I'd had 
...
- Short-wave infrared images come in three times in the first seven variables, and an RGB image doesn't 
come in until the 5th place. SWIR often comes out as highly important.

## Example map

This map shows the prediction from the same model, zoomed in on a small area (about 1 ha) in Old Town Hill.
...
collected in 2019-20; imagery is from 2018-21). To some extent, the model is preferring years that match well,
but I got a better fit restricting to field years plus or minus one. Strictly matching years reduces the amount
of available imagery. This will be worth exploring.
...
2. Assess spatial autocorrelation and decide how sparsely we need to sample the data. Run with more complete
...
of choices, a few of which will be appropriate.
...
2. Make data sampling more flexible.
2. Deal better with missing values in source data. Automatically reject images with too much missing data
in site footprint.
2. Screen source imagery for missing data and other issues. Pick the best image for each season/year/sensor.
2. Improve robustness to various data issues.
...
2. Try applying models across sites and building models for multiple sites in a cross-validation framework.
