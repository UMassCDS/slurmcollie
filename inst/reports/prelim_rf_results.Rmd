...
```{r}
model_fit$confuse$byClass$AUC <- aucs(model_fit$fit)
print(model_fit$confuse)
auc_f1 <- t(data.frame(AUC = saltmarsh::aucs(model_fit$fit), F1 = 1:19 / 19))
print(auc_f1)
```
#### Note
...
- For each class, we get the following statistics. These all scale from 0 to 1, with 1 generally indicating
  higher performance (except for prevalence, detection rate, and detection prevalence).
  - *Sensitivity*, true positives / all positives (also called *precision*)
...
7. Improve robustness to various data issues.
